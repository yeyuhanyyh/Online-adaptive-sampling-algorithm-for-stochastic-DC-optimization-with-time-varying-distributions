...
function g_k_func = compute_g(beta, xi, yi, alpha, N, lambda)
    ...
    % 计算 Huber 损失
    huber_loss = (abs(residual) <= delta) .* (0.5 * residual.^2) + ...
                 (abs(residual) > delta) .* (delta * (abs(residual) - 0.5 * delta));
    
    % 总目标函数值
    g_k_func = (1 + alpha * norm(beta, 1)) * lambda + mean(huber_loss); % Huber 损失取均值
end
